  ...
  const [newClass, setNewClass] = useState({
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:8080/api/classes', {
            ...
            .then((data) => {
                // Function to update dictionaries and ensure "batch" is the first attribute
                function updateList(list) {
                    for (let i = 0; i < list.length; i++) {
                        list[i] = {id: i + 1, batch: i + 1, ...list[i] };
                    }
                    return list;
                }

// Update the list to include "batch" as the first attribute with serial numbers
                let updatedData = updateList(data);

// Output the updated list of dictionaries
                console.log(updatedData);
                setdata(updatedData); // Set the fixtures state with the received data
                console.log(data);
            })
            .catch((error) => {
                ...
                <tr key={row.id}>
                  ...
                  <div className="flex flex-col">
                    <Link
                      ...
                      to={`/upload?course=${row.class}&batch=${row.batch}&strength=${row.strength}`}
                    >
                      <button
                        id="click"
                        onClick={directpage}
